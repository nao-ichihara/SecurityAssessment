...
### 5.10 多言語対応（i18n）フェーズ1

| 項目 | 内容 |
|---|---|
| 対応言語 | 日本語（`ja`）・英語（`en`） |
| 言語バー | メニューバー右端に「日本語 \| ENGLISH」ボタン配置（`margin-left:auto`） |
| i18n辞書 | `const I18N = { ja:{...}, en:{...} }` を `index.html` 内に定義。ja/en 各165キー |
| `t(key)` | 現在言語の文字列を返す。未定義キーは `ja` にフォールバック |
| `applyI18n()` | `data-i18n` / `data-i18n-ph` / `data-i18n-title` / `data-i18n-html` 属性を持つ全DOM要素を一括更新 |
| `switchLang(lang)` | 言語切替・localStorage保存・`applyI18n()` 実行・現在画面を再描画 |
| 言語設定の永続化 | `localStorage.setItem('lang', lang)` で保存、起動時に復元 |
| 対応外 | 質問テキスト・識別番号定義（Phase 3〜4 で対応予定）。利用者入力値（メモ・コメント・組織情報）は言語切替の影響なし |

**日付フォーマット：**

| 言語 | ラベル表示 | 日付表示例 |
|---|---|---|
| 日本語 | 評価実施日 | 2026年3月21日 |
| 英語 | Assessment Date (MM/DD/YYYY) | Mar 21, 2026 |

### 5.11 回答進捗ダッシュボード

回答入力画面（Step 1）の情報バー直下に常時表示される進捗ウィジェット。

| 要素 | ID | 内容 |
|---|---|---|
| 回答率メーターバー | `dashboardBar` | 全質問数に対する回答済み割合をグラデーションバーで表示 |
| 回答率% | `dashboardPct` | 回答率を数値 `xx%` で表示 |
| セクション問数 | `dashboardSectionCount` | 現在セクションの質問数 / 全質問数（例：`12 / 90`） |
| 全問数 | `dashboardTotalCount` | 回答済み数 / 全質問数（例：`28 / 90`） |

**更新タイミング：** `updateProgress()`（回答変更時）・`updateNav()`（セクション移動時）で連動更新。
...
| v2.8 | 2026-03-21 | 多言語対応フェーズ1実装（日本語/英語 UIラベル動的切替）。言語バーをメニューバー右端に追加。i18n辞書（ja/en 各165キー）・`t()`・`applyI18n()`・`switchLang()` 実装。英語モード日付フォーマットヒント追記。言語設定のlocalStorage永続化。 |
| v2.9 | 2026-03-21 | 回答進捗ダッシュボード追加（回答率%メーター・セクション問数・全問数カウンター）。各種バグ修正（フレームワーク一覧未表示・全画面同時表示・英語モード混在文字列・言語バー位置）。解説書に言語切替・進捗ダッシュボードの説明を追加。 |
